...
if (isset($_POST['author']) && !empty($_POST['author']) && isset($_POST['message']) && !empty($_POST['message']) && isset($_POST['tour_operator_id']) && !empty($_POST['tour_operator_id']) && isset($_POST['grade_total'])) {
    ...
    if (!empty($_POST['grade_total'])) {
        ...
        $tourOperator->setGradeTotal($tourOperator->getGradeTotal() + $grade_total);
...
<body>
    ...
    <?php
        if ($tourOperator) {
            ...
            echo '<p> Note : ' . $tourOperator->getGradeCount() . '</p>';
            echo '<p> Note totale : ' . $tourOperator->getGradeTotal() . '</p>';
            echo '<div class="logo" style="background:url('.$tourOperator->getImg().')center no-repeat fixed; background-size:contain;"></div>';
        }
    ?>
    </div>
<div class="col-12 col-md-7 mb-5 content">
    <?php foreach($destinations as $destination): ?>
        <div class="destination" style="background:url('<?php echo $destination->getImg(); ?>')top center no-repeat; background-size:cover;">
